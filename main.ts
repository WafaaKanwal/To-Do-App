...
//Update Tasks
  ...
  if (answer.options === chalk.magentaBright("Update Tasks")) {
    ...
    let updateIndex = Number(
      prompt(chalk.greenBright("Enter the index number of the task you want to update: "))
    ) -1;
  ...
  //Add Tasks
    ...
    let input = prompt(
      chalk.greenBright(chalk.greenBright("Enter your task here: "))
    );
    ...
    let deleteIndex = Number(
      prompt(
        chalk.greenBright(
          "Enter Index number of task that you want to delete: "
        )
      )
    ) -1;
